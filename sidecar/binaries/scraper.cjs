...
var matchSchema = external_exports.object({
  matches: external_exports.array(
    external_exports.object({
      listing_id: external_exports.string().describe("The listing ID from the input"),
      keyword: external_exports.string().describe("The matched keyword"),
      relevance: external_exports.number().min(0).max(1).describe("Relevance score: 0 = not relevant, 1 = highly relevant")
    })
  )
});
function buildPrompt(listings, keywords, threshold) {
  const listingTexts = listings.map(
    (l) => `[${l.id}] ${l.title} | ${l.location} | ${l.surface || "?"}m\xB2 | ${l.rooms || "?"} rooms | ${l.price != null ? `${l.price}\u20AC` : "?"}`
  ).join("\n");
...
function getModel(provider, apiKey, model, ollamaEndpoint) {
  switch (provider) {
    case "openai": {
...
async function matchKeywords(listings, keywords, provider, apiKey, model, ollamaEndpoint, relevanceThreshold) {
...
function normalizeListings(raw) {
  return raw.filter((l) => l.list_id).map((l) => {
    const attr = l.attributes || [];
    const getAttr = (key) => attr.find((a) => a.key === key)?.value_label || attr.find((a) => a.key === key)?.value || null;
...
async function scrape(payload) {
  ...
  if (payload.chrome_profile) {
    context = await import_playwright_core.chromium.launchPersistentContext(payload.chrome_profile, {
  ...
  try {
    await page.goto(payload.url, { waitUntil: "domcontentloaded", timeout: 3e4 });
    const waitMs = interactive ? 1e4 : 2e3;
...
async function extractListings(page, payload) {
  ...
  const rawListings = searchData?.ads || searchData?.listings || [];
...
async function main() {
  ...
  const output = await scrape(payload);
